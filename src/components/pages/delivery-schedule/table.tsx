import { Trip } from "@/features/api/schedule-module/deliverySchedule.type";
import { Button } from "rizzui";

interface TableProps {
  deliveryDetails: Trip;
}
        ...
        <thead>
          <tr className="text-base h-[40px] text-[#434343] bg-[#C6E7D9] sticky top-0 left-4 z-10 rounded-md whitespace-nowrap">
            {[
              ...
              <th
                key={header}
                className=" flex-1 font-normal first:pl-5 px-2 text-center border-y-[1px] border-y-primary"
              >
                {header}
              </th>
            ))}
